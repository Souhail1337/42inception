...
function allow_subdomain_install() {
	...
	if ( parse_url( get_option( 'home' ), PHP_URL_PATH ) || 'sel-fcht.42.fr' === $domain || preg_match( '|^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$|', $domain ) ) {
		return false;
	}
...
function network_step1( $errors = false ) {
	...
	if ( $is_www ) :
		...
		<table class="form-table" role="presentation">
		<?php if ( 'sel-fcht.42.fr' === $hostname ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Installation' ); ?></th>
				<td>
				<?php
					printf(
						/* translators: 1: sel-fcht.42.fr, 2: sel-fcht.42.fr.localdomain */
						...
						'<code>sel-fcht.42.fr</code>',
						'<code>sel-fcht.42.fr.localdomain</code>'
					);
					// Uh oh:
